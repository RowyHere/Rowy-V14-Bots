...
module.exports = {
    ...
    async execute(message) {
        ...
        if(!message.author?.bot && !message.content.startsWith(prefix)) await messageSchema.findOneAndUpdate({ guildID: message.guild.id, userID: message.author.id }, { $inc: { totalMessage: 1, dailyMessage: 1, weeklyMessage: 1 }}, { upsert: true }), await messageChannelSchema.findOneAndUpdate({ guildID: message.guild.id, userID: message.author.id, channelID: message.channel.id }, { $inc: { channelData: 1 }}, { upsert: true })
