    ...
    async execute(client, message, args, embed) {
        ...
        async function cmute (user, reason, time, timeLength, button, count) {
            ...
            await button.channel.send({ embeds: [
                embed.setDescription(`${message.member}, ${user} adlı kullanıcı başarıyla **metin** kanallarında susturuldu!`)
            ] })
            ...
            cmute.wsend({
                embeds: [
                    ...
                    .setTimestamp()
                    .setDescription(`${user} (\`${user.id}\`) adlı kullanıcı ${button.user} tarafından metin kanallarında susturuldu.`)
                    .addFields({
